...
export default {
        ...
        innerValue: {
            get() {
                ...
                const realVal = !this.labelInValue
                    ? valueAsArr
                    : valueAsArr?.map((item) => item[this.labelInValue?.value ?? 'value']) ?? [];

                if (this.multiple) {
                    return realVal;
                }

                return isEmpty(realVal[0]) && this.mergedHasAll ? this.valueOfAll : realVal[0];
            },
            set(newValue) {
                if (!this.labelInValue || newValue === this.valueOfAll || isEmpty(newValue)) {
                    this.$emit('update:modelValue', newValue === this.valueOfAll ? null : newValue);
                    return;
                }

                const valueAsArr = toArray(newValue);
                const target = this.mergedOptions?.filter((option) => valueAsArr.includes(option.value)) ?? [];
                ...
                this.$emit('update:modelValue', this.multiple ? emitValue : emitValue[0]);
            },
        },
        /** 是否预览状态 */
        isPreview() {
    ...
    render() {
        ...
        const children = this.mergedOptions.map((option) => {
            ...
            const optionChildren =
                option.children?.map((child) => {
                    ...
                    return h(resolveComponent('ElOption'), childAttribute);
                }) ?? [];
            return h(resolveComponent(hasGroup ? 'ElOptionGroup' : 'ElOption'), optionAttribute, {
        ...
        const attribute = {
            ...
            class: `oms-select ${this.$attrs.class || ''}`,
            ref: 'omsSelectRef',
        };

        return h(resolveComponent('ElSelect'), attribute, {
