<template>
    <div class="oms-page">
        ...
        <oms-table
            ...
            v-on="tableListeners"
        >
            <template v-for="slotName in Object.keys($slots)" v-slot:[slotName]="attrs">
                <slot :name="slotName" v-bind="attrs"></slot>
            </template>
        </oms-table>
        <oms-modal v-bind="modalProps" v-model:visible="panelVisible">
            <template v-for="slotName in Object.keys($slots)" v-slot:[slotName]="attrs">
                <slot :name="slotName" v-bind="attrs"></slot>
            </template>
        </oms-modal>
...
import OmsFilter from './filter.vue';
import OmsTable from './table.vue';
import OmsModal from './modal.vue';
...
export default {
    ...
    computed: {
        /** 列表组件通用属性透传 */
        tableProps() {
            const results = Object.keys(tablePropsMixins.props).reduce((prev, curr) => {
                return {
                    ...prev,
                    [curr]: this[curr],
                };
        ...
        /** 列表组件通用属性透传 */
        filterProps() {
            const results = Object.keys(filterPropsMixins.props).reduce((prev, curr) => {
                return {
                    ...prev,
                    [curr]: this[curr],
                };
        ...
        /** filter事件透传 */
        filterListeners() {
            return mapKeys(pick(this.$attrs, []), (_, key) => key.replace('onOn', 'on'));
        },
        /** table事件透传 */
        tableListeners() {
            return mapKeys(pick(this.$attrs, []), (_, key) => key.replace('onOn', 'on'));
    ...
    created() {
        ...
        this.$emitter.on('OmsFormPageOnCreate', (_, options = {}) => {
            this.onAction('create', { defaultValues: {}, ...options });
        });
        this.$emitter.on('OmsFormPageOnUpdate', (defaultValues = {}, options = {}) => {
            this.onAction('edit', { defaultValues, ...options });
        });
        this.$emitter.on('OmsFormPageOnDetail', async (defaultValues = {}, options = {}) => {
            this.onAction('detail', { defaultValues, ...options });
        });
        this.$emitter.on('OmsFormPageUsePageModal', async (defaultValues = {}, options = {}) => {
            this.onAction('usePageModal', { defaultValues, ...options });
        });
        this.$emitter.on('OmsFormPageOnRemove', async (id) => {
    ...
    methods: {
        ...
        onAction(actionType = 'create', options = {}) {
            this.actionParams = {
                ...
                status: ['detail', 'usePageModal'].includes(actionType) ? 'preview' : 'edit',
                actions: ['detail', 'usePageModal'].includes(actionType) ? '' : undefined,
                formatBefore: actionType === 'create' ? null : this.detailProps.formatBefore,
                actionType,
                onOk: async (formValues) => {
                    await this[actionType === 'create' ? 'onCreate' : 'onUpdate']?.(formValues);
                    this.refresh();
                },
                ...options,
            };
